function [nn, L]  = nntrain(nn, train_x, train_y, opts, val_x, val_y, train_w, val_w, weights)
...
for i = 1 : numepochs
    ...
    for l = 1 : numbatches
        ...
        if isempty(weights) || i<10
            batch_weights = [];
        else
            batch_weights = weights(kk((l - 1) * batchsize + 1 : l * batchsize), :);
        end
        
        nn = nnff(nn, batch_x, batch_y);
        nn = nnbp(nn, batch_weights);
        nn = nnapplygrads(nn);
